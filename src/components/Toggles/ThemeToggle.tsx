...
const ThemeToggle = () => {
  ...
  useEffect(() => {
    ...
    document.body.classList.add(defaultTheme === 'dark' ? 'dark-theme' : 'light-theme');
  }, []);
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`theme-toggle relative w-16 h-9 p-1 cursor-pointer rounded-full transition-colors ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-400'
      }`}
      onClick={toggleTheme}
      onKeyDown={(e) => e.key === 'Enter' && toggleTheme()}
    >
      <div
      className={`theme-toggle-circle absolute top-1/2 left-1 w-7 h-7 rounded-full shadow transform transition-transform duration-500 ${theme === 'dark'
      ...
      {/* Sun Icon */}
      <svg
      className={`absolute top-1/2 left-2 w-5 h-5 transform transition-opacity duration-500 ${theme === 'dark' ? 'opacity-0' : 'opacity-100'
        } -translate-y-1/2`}
      viewBox="0 0 24 24"
      fill="none"
      ...
      {/* Moon Icon */}
      <svg
      className={`absolute top-1/2 right-2 w-5 h-5 transform transition-opacity duration-500 ${theme === 'dark' ? 'opacity-100' : 'opacity-0'
        } -translate-y-1/2`}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
